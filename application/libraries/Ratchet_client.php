...
class Server implements MessageComponentInterface
{
        ...
        if (!empty($datas->user_id)){
            ...
            if (!empty($datas->user_id) && $datas->user_id !== $client->resourceId) {
                ...
                if (!empty($this->CI->ratchet_client->callback['auth']) && empty($client->subscriber_id)) {
                    ...
                    $this->CI->ratchet_client->room_id = $datas->room_id;
                    $this->CI->ratchet_client->user_id = $client->subscriber_id;


                    $room_info = $this->CI->m_api->selectDebateRoomOne($datas->room_id);
                    if (!empty($room_info)){

                        $room_connect = $this->CI->m_api->selectDebateRoomConnect($datas->room_id);

                        if ($room_info['member_cnt'] > count($room_connect)){

                            #내가 있는지 체크 후 insert
                            $check = $this->CI->m_api->selectDebateRoomConnectByUser($datas->room_id, $client->subscriber_id);
                            if (empty($check)){
                                $data = array("debate_room_idx" => $datas->room_id , "user_idx" => $client->subscriber_id , "reg_date" => date('Y-m-d h:i:s'));
                                $this->CI->m_api->insertDebateRoomConnect($data);

                                $client->room_id = $datas->room_id;
                            }
    ...
    public function onClose(ConnectionInterface $connection)
    {
        ...
        $this->CI->m_api->deleteDebateRoomConnect($this->CI->ratchet_client->room_id, $this->CI->ratchet_client->user_id );
